...
int main(int argc, char const *argv[]){
	...
	Tri_t GoldenTriangle = {0};		//Initial Triangle
	Tri_t HiddenTriangle = {0};		//Triangle We're Eventually Gnna Find
	...
	for (char i = 0; i < 3; ++i) SideDistances[i] = Px2_Dist(GoldenTriangle.pts[i], GoldenTriangle.pts[(i+1)%3]);
	//calculate distances between the golden triangle points

	if(PtAmt<4){puts("Less Than 4 Points");return 0;}			//not rly supposed to be < 4

	PlanePts = (Px2_t*)malloc(PtAmt*sizeof(Px2_t));	//allocate space for other points
	for (unsigned i = 0; i < PtAmt; ++i) scanf("%d %d", &PlanePts[i].x, &PlanePts[i].y);
	//read points

	for (unsigned f = 0; f < PtAmt; ++f){				//find a -> b or vice versa
		for (unsigned s = f+1; s < PtAmt; ++s){
	...
	for (int rtry = 0; rtry < 2; ++rtry){
		for (unsigned i = 0; i < PtAmt; ++i){				//find a -> c
	...
	qsort(&HiddenTriangle, 3, sizeof(Px2_t), (int (*)(const void*,const void*))qsort_2dPt);
